...
import ClientBranchController from './client-branch-controller'

const PATH_PREFIX: string = '/client-branch'

export default class ClientBranchRoutes extends AbstractRoutes {
  public route() {
    this.router.post(`${PATH_PREFIX}/search`, async (req: Request, res: Response) => {
      try {
        const controller = new ClientBranchController((getMongoConnection()))
    ...
    this.router.post(`${PATH_PREFIX}/:id/reach`, async (req: Request, res: Response) => {
      try {
        const controller = new ClientBranchController((getMongoConnection()))
        const data = await controller.addReach({
          data: {
            ...req.body,
            by: (req as any).by,
          },
    ...
    this.router.put(`${PATH_PREFIX}/:id/reach/:reachId`, async (req: Request, res: Response) => {
      try {
        const controller = new ClientBranchController((getMongoConnection()))
        const data = await controller.updateReach({
          data: {
            ...req.body,
            updatedBy: (req as any).by,
            updatedTs: new Date(),
