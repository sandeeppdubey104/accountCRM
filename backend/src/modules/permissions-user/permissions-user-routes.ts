...
import PermissionsUserController from './permissions-user-controller'

const PATH_PREFIX: string = '/permissions-user'

export default class PermissionsUserRoutes extends AbstractRoutes {
  public route() {
    this.router.post(`${PATH_PREFIX}/search`, async (req: Request, res: Response) => {
      try {
        const controller = new PermissionsUserController((getMongoConnection()))
