...
const PATH_PREFIX: string = '/company-branch'

export default class CompanyBranchRoutes extends AbstractRoutes {
  public route() {
    this.router.post(`${PATH_PREFIX}/search`, async (req: Request, res: Response) => {
      try {
        const controller = new CompanyBranchController((getMongoConnection()))
