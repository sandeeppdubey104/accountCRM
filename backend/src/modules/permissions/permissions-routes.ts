...
const PATH_PREFIX: string = '/permissions'

export default class PermissionsRoutes extends AbstractRoutes {
  public route() {
    this.router.post(`${PATH_PREFIX}/search`, async (req: Request, res: Response) => {
      try {
        const controller = new PermissionsController((getMongoConnection()))
